...
export const Header = () => {
  ...
  return (
    <Box sx={{ width: '100%', backgroundColor: 'background.default', borderBottom: 1, borderColor: 'divider' }}>
      <AppBar position="static" elevation={0} sx={{ backgroundColor: 'background.default', boxShadow: 'none' }}>
        <Toolbar sx={{ minHeight: { xs: 56, md: 80 }, px: { md: 6 }, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <IconButton edge="start" color="inherit" sx={{ display: { md: 'none' }, mr: 1 }} onClick={handleDrawer(true)}>
            <MenuIcon />
          </IconButton>
          <Link href="/" passHref>
            <Box component="a" sx={{ display: 'flex', alignItems: 'center', minWidth: 140, mr: 3, textDecoration: 'none', cursor: 'pointer' }}>
              <Box sx={{ width: 28, height: 28, background: theme.palette.common.white, borderRadius: 1, mr: 1 }} />
              <Typography variant="h6" fontWeight={700} color="common.white" sx={{ userSelect: 'none' }}>
                Logo
              </Typography>
            </Box>
          </Link>
                ...
                menu.submenu ? (
                  <Box key={menu.label} sx={{ display: 'flex', alignItems: 'center' }}>
                    <Button
                      ...
                      sx={{ textTransform: 'none', fontSize: 16, fontWeight: 400, color: 'common.white', pl: 2, pr: 2 }}
                      endIcon={<KeyboardArrowDownIcon sx={{ fontSize: 20, color: 'common.white' }} />}
                    >
                      {menu.label}
                    </Button>
                    <Menu
                      ...
                      MenuListProps={{ sx: { background: 'background.paper' } }}
                      disableScrollLock
                    >
                      {menu.submenu.map((item) => (
                        <MenuItem key={item} onClick={handleMenuClose} sx={{ color: 'text.primary' }}>
                          {item}
                        </MenuItem>
                      ))}
                    </Menu>
        ...
        <Drawer
          ...
          PaperProps={{ sx: { width: 260, background: 'background.default', color: 'text.primary' } }}
        >
